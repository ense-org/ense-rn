...
import CrossFadeIcon from './TabBarIcon';
...
export type BottomBarProps = TabBarOptions & {
  navigation: any,
  onTabPress: any,
  onTabLongPress: any,
...
class TabBarBottom extends React.Component<BottomBarProps, State> {
  static defaultProps = {
  ...
  _icons = () => {
    ...
    return routes.map((route, index) => {
      ...
      const accessibilityRole = this.props.getAccessibilityRole({ route });
  ...
  render() {
    const { keyboardHidesTabBar, safeAreaInset, style, centerView } = this.props;
    const view = this._icons();
    if (centerView) {
      view.splice(view.length / 2, 0, centerView);
    }
